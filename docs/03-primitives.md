...
Choice는 최대 255개 선택지를 지원합니다. 선택지를 늘리면 input token이 늘어나므로 설명 길이와 함께 고려합니다. 자세한 수치는 [모델과 한도 문서](./12-limits-reference.md)를 참고합니다.
...
Score는 최소 2개, 최대 10개 수준을 사용합니다. 수준이 1개이면 SDK 타입 단계에서 오류가 발생합니다. 수준을 늘리기 전에 각 단계를 실제로 구분해 설명할 수 있는지 확인합니다.
